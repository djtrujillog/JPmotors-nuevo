...
const AutoModal = ({ auto, onClose }) => {
  ...
  const handleCotizacionSubmit = async (e) => {
    e.preventDefault();
    const response = await fetch('https://jpmotorsgtimg-afa7fve9gmarguep.centralus-01.azurewebsites.net/mail/cotizacion', {
      method: 'POST',
      headers: {
    ...
    const fetchData = async () => {
      try {
        const [
          ...
          fetch(`https://jpmotorsgtimg-afa7fve9gmarguep.centralus-01.azurewebsites.net/marcas/${auto.MarcaID}`),
          ...
          fetch(`https://jpmotorsgtimg-afa7fve9gmarguep.centralus-01.azurewebsites.net/vehiculos/detalleGarantia/${auto.VehiculoID}`)
        ]);
