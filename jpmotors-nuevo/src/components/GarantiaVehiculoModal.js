...
const DetallesVehiculoGarantia = ({ show, handleClose, vehiculo }) => {
  ...
  const fetchExistingDetalles = async (VehiculoID) => {
    try {
      const response = await axios.get(`https://jpmotorsgt.azurewebsites.net/vehiculos/detalleGarantia/${VehiculoID}`);
      if (response.data) {
  ...
  const handleEditDetalle = async () => {
    try {
      ...
      await axios.put(`https://jpmotorsgt.azurewebsites.net/vehiculos/detalleGarantia/${encodeURIComponent(originalDetalle)}`, body);
  ...
  const handleAgregarDetalle = async () => {
    try {
      ...
      const response = await axios.post('https://jpmotorsgt.azurewebsites.net/vehiculos/detalleGarantia', body);
  ...
  return (
    <Modal show={show} onHide={handleClose} size="xl">
      <Modal.Header closeButton>
        <Modal.Title>Terminos de Garantia del vehiculo</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {vehiculo ? (
          <>
            ...
            <ListGroup>
              {existingDetalles.length > 0 ? (
                ...
                <p>Sin terminos de garantia</p>
              )}
            </ListGroup>
            <h3>Agregar terminos de garantia</h3>
            <Form inline>
              <Form.Control 
                type="text" 
                value={newDetalle} 
...
export default DetallesVehiculoGarantia;
