...
const DetallesVehiculoInteriorModal = ({ show, handleClose, vehiculo }) => {
  ...
  const fetchExistingDetalles = async (VehiculoID) => {
    try {
      const response = await axios.get(`https://cotizaciones-jpmotors.onrender.com/vehiculos/detalleInterior/${VehiculoID}`);
      if (response.data) {
  ...
  const handleEditDetalle = async () => {
    try {
      ...
      await axios.put(`https://cotizaciones-jpmotors.onrender.com/vehiculos/detalleInterior/${encodeURIComponent(originalDetalle)}`, body);
  ...
  const handleAgregarDetalle = async () => {
    try {
      ...
      const response = await axios.post('https://cotizaciones-jpmotors.onrender.com/vehiculos/detalleInterior', body);
