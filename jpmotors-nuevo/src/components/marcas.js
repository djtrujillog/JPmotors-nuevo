...
function ListaMarcas() {
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm("¿Estás seguro de que deseas eliminar esta marca?")) {
        try {
            await axios.delete(`https://jpmotorsgt.azurewebsites.net/marcas/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`https://jpmotorsgt.azurewebsites.net/marcas/${marcaActual.MarcaID}/logo`, formData); // Actualiza la marca


      } else {
        await axios.post("https://jpmotorsgt.azurewebsites.net/marcas/", formData);
      }
      fetchMarcas();
      handleCloseModal();
    } catch (error) {
